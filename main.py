...
def calculate_option_prices(option_type_var, stock_price_entry, strike_price_entry, time_to_expiration_entry, volatility_entry, yield_label, option_type_label, call_price_label, put_price_label):
    try:
        # Get user input from entry boxes
        option_type = option_type_var  # Get input, remove whitespace, and uppercase
        print("Option type: ",option_type)
        stock_price = float(stock_price_entry)
        strike_price = float(strike_price_entry)
        time_to_expiration = float(time_to_expiration_entry) / 365  # Convert days to years
        volatility = float(volatility_entry)
...
def create_gui():
    ...
    # Calculate Button
    calculate_button = Button(window, text="Calculate", command=lambda: calculate_option_prices(option_type_var.get(), stock_price_entry.get(), strike_price_entry.get(), time_to_expiration_entry.get(), volatility_entry.get(), yield_label, option_type_label, call_price_label, put_price_label))
